...
####################################### Drop age 
df_credit = df_credit.drop(['Age_cat_Young','Age_cat_Adult','Age_cat_Senior'], 1)
X = df_credit.drop('Risk_bad', 1).values
y = df_credit["Risk_bad"].values
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.25, random_state=42)
...
####################################### Drop sex 
df_credit = df_credit.drop('Sex_male', 1)
X = df_credit.drop('Risk_bad', 1).values
y = df_credit["Risk_bad"].values
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.25, random_state=42)

model.fit(X_train, y_train)
y_pred_drop_sex = model.predict(X_test)

print('len:',len(y_pred))
print('Results:', ( sum(y_pred != y_pred_drop_sex) / len(y_pred)))

print("\n accuracy_score")
print(accuracy_score(y_test,y_pred_drop_sex))
print("\n confusion matrix")
print(confusion_matrix(y_test, y_pred_drop_sex))
print("\n Classification report")
print(classification_report(y_test, y_pred_drop_sex))
print("\n fbeta_score")
print(fbeta_score(y_test, y_pred_drop_sex, beta=2))
